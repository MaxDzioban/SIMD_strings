...
void test_copy_speed() {
    ...
    std::cout << "| Size (chars) | my_str_t avg time (μs) | my_str_simd avg time (μs) |\n";
    std::cout << "|--------------|------------------------|---------------------------|\n";
    ...
    for (size_t size : sizes) {
        ...
        for (int i = 0; i < warmup_runs; ++i) {
            ...
            my_str_simd warmup_simd(source);
...
void test_reserve_speed() {
    ...
    std::cout << "my_str_simd reserve time: " << simd_time << " microseconds\n";
...
void test_operator_eq_one(const char* expected, const my_str_simd& actual, int repeat = 1) {
    auto start = std::chrono::high_resolution_clock::now();
    bool result = true;
...
// для my_str_simd
void test_operator_eq_one_simd(const char* expected, const my_str_simd& actual, int repeat = 1) {
    auto start = std::chrono::high_resolution_clock::now();
    bool result = true;
    for (int i = 0; i < repeat; ++i) {
        result &= (expected == actual);
...
void test_operator_eq_simd(const my_str_simd& str1, const char* cstr2, int repeat = 1) {
    auto start = std::chrono::high_resolution_clock::now();
    bool result = true;
    for (int i = 0; i < repeat; ++i) {
        result &= (str1 == cstr2);
...
void test_operator_eq_simd(const my_str_simd& str1, const my_str_simd& str2, int repeat = 1) {
    auto start = std::chrono::high_resolution_clock::now();
    bool result = true;
    for (int i = 0; i < repeat; ++i) {
        result &= (str1 == str2);
    ...
    if (!result) {
        std::cerr << "SIMD operator==(my_str_simd, my_str_simd) test failed!\n";
    } else {
        std::cout << "SIMD operator==(my_str_simd, my_str_simd) test passed.\n";
    }
    std::cout << "Time taken (" << repeat << "x): " << elapsed << " microseconds\n\n";
}
...
void test_resize_speed_different_sizes() {
    ...
    for (size_t initial_size : sizes) {
        ...
        std::cout << "my_str_simd resize time: " << time_simd << " microseconds\n";
...
void test_insert_speed_different_sizes() {
    ...
    for (size_t base_size : base_sizes) {
        ...
        my_str_simd base_simd(base_source);
        my_str_simd insert_simd(insert_source);
        ...
        auto simd_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        std::cout << "my_str_simd insert time: " << simd_time << " microseconds\n";
...
void test_erase_speed_different_sizes() {
    ...
    for (size_t base_size : base_sizes) {
        ...
        auto simd_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        std::cout << "my_str_simd erase time: " << simd_time << " microseconds\n";
...
void test_find_speed() {
    ...
    for (size_t size : sizes) {
        ...
        auto simd_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        std::cout << "my_str_simd find time: " << simd_time << " microseconds\n";
...
void test_insert_speed() {
    ...
    for (size_t size : sizes) {
        ...
        volatile size_t checksum = 0;
        my_str_simd str(source);
        my_str_simd insert_str(insert_str_content);
...
int main() {
    ...
    my_str_simd test_heloo("Hello, C++ World!");
    const char* compare111 = "Hello, C++ World!";
    my_str_simd test2222("Hello everyone, World!");
    const char* compare222 = "Hello everyone, World!";
    std::string bigwww_string(10'000'000, 'x');
    my_str_simd bigwe_test(big_string);
    const char* bigwe_compare = big_string.c_str();
    std::cout << "Testing operator==(my_str_simd, const char*):\n";
    ...
    my_str_simd test1_eq_eq("Hello, SIMD World!");
    my_str_simd test2_eq_eq("Hello, SIMD World!");
    my_str_simd big_test1(std::string(10'000'000, 'x'));
    my_str_simd big_test2(std::string(10'000'000, 'x'));
    std::cout << "Testing operator==(my_str_simd, my_str_simd):\n";
